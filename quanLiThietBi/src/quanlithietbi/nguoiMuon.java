...
public class nguoiMuon extends javax.swing.JFrame {
    ...
    public void loadData() {
        ...
        try {
            ConnectionDB1 a = new ConnectionDB1();
            Connection cnn = a.getConnectDB();
            int number;
            Vector row, column;
            column = new Vector();
            Statement st = cnn.createStatement();
            ResultSet rs = st.executeQuery("select * from NguoiMuon");
    ...
    private void initComponents() {
        ...
        setTitle("Quản lý người mượn_Bùi Thị Mến-Lê Anh Tuấn-Phan Văn Tài");
        ...
        Table_DG.setForeground(new java.awt.Color(0, 51, 153));
        ...
        jLabel10.setText("Quản lý người mượn");
    ...
    private void o_themActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_o_themActionPerformed
        ...
        try {
            ConnectionDB1 a = new ConnectionDB1();
            Connection cnn = a.getConnectDB();
            PreparedStatement ps = cnn.prepareStatement("insert into NguoiMuon values(?,?,?,?,?,?,?,?) ");
    ...
    private void o_xoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_o_xoaActionPerformed
        try {
            ConnectionDB1 a = new ConnectionDB1();
            Connection cnn = a.getConnectDB();
            PreparedStatement comm = cnn.prepareStatement("DELETE FROM ChiTietMuonTra\n"
                    + " WHERE [MaMuonTra ] IN\n"
                    + "(SELECT [MaMuonTra ] FROM MuonTra \n"
                    + "WHERE [MaNguoiMuon]= ? )\n"
                    + "DELETE FROM MuonTra\n"
                    + "WHERE [MaNguoiMuon]= ?\n"
    ...
    private void o_suaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_o_suaActionPerformed
        try {
            ConnectionDB1 a = new ConnectionDB1();
            Connection cnn = a.getConnectDB();
            PreparedStatement comm = cnn.prepareStatement("Update NguoiMuon set [TenNguoiMuon] =?,[Ngaysinh]=?,[GioiTinh]=?,[SoCMND]=?,[SoDienThoai]=?,[Email]=?,[QueQuan]=? where [MaNguoiMuon]=? ");
